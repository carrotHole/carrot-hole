...
public class AuLoginServiceImpl implements AuLoginService {
    ...
    @Autowired
    private AuTenantService tenantService;

    @Override
    public LoginResultVO login(LoginVO vo) {
        // 通过租户标志获取租户主键
        AuTenant tenant = CheckUtil.checkNotNull(tenantService.getByTenantMark(vo.getTenantMark()), "未找到当前租户信息");
        List<AuUser> users = CheckUtil.checkNotNull(userService.ListUserByUserName(vo.getUsername(), tenant.getId()), new AuthorizationException("用户名或密码错误"));
        ...
        List<AuDept> depts = deptService.listByIds(deptIds, tenant.getId());
    ...
    public String createToken(String identifyKey, String deptId) {
        CacheUtil.CacheValue cacheValue = CacheUtil.removeCache(CacheKeyUtil.getIdentifierKey(identifyKey));
        ...
        AuUserResultVO auUser = CheckUtil.checkNotNull(userService.getOneByUsernameDeptAs(vo.getUsername(), deptId, vo.getTenantId()), "登录token异常");
        // 创建token
